...
for i in $(seq 1 ${max_jobs}); do
	...
	echo "#SBATCH --partition=guo-neurostat" >> script$i.out
	...
	echo "R CMD BATCH '--args job_control='run',iter=$i' ${script}" >> script$i.out
...
for j in $(seq 1 $nloops); do
	for i in $(seq 1 ${max_jobs}); do
		...
		echo "#SBATCH --partition=guo-neurostat" >> script$jid.out
		...
		echo "R CMD BATCH '--args job_control='run',iter=$jid' ${script}" >> script$jid.out
